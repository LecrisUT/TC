...
void Method::set_calc_method(const std::string &calc_method) {
    ...
    error_messages::compare_list(
        "method", calc_method,
        list_method); // If not included in list_method, stop the program
...
void Method::check_consistency_of_calc_mode(
    const std::string &calc_mode_QE) const {
    if (!((calc_mode_QE == "scf" && calc_mode_ == "SCF") ||
          (calc_mode_QE == "bands" && calc_mode_ == "BAND"))) {
        error_messages::stop("calc_mode (input.in) and calculation in QE "
                             "('scf' or 'bands') are not consistent.");
    }
}

...
void io_tc_files::dump_scfinfo(const FileNames &file_names,
                               ...
                               std::ostream *ost) {
    *ost << " Dump SCF informations (" << file_names.tc_scfinfo() << ")"
         << std::endl;
    std::ofstream ofs(file_names.tc_scfinfo(),
    ...
    std::vector<int> num_sym_at_k(
        num_irreducible_kpoints_scf); // number of symmetrically equivalent
                                      // k-points for each irreducible k-point
    for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
        num_sym_at_k[ik] = kpoints.kvectors_scf()[ik].size();
        ofs.write(reinterpret_cast<const char *>(&num_sym_at_k[ik]),
                  sizeof(int));
    }

    std::vector<int> size_FFT_grid_vec =
        plane_wave_basis
            .size_FFT_grid_vec(); // to check consistency of the FFT grid
    ofs.write(reinterpret_cast<const char *>(&size_FFT_grid_vec[0]),
              sizeof(int) * 3);
    ...
    for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
        for (int isym = 0; isym < num_sym_at_k[ik]; isym++) {
            for (int idim = 0; idim < 3; idim++) {
                ...
                ofs.write(reinterpret_cast<const char *>(&kvectors_scf_i),
                          sizeof(double));
            }
            int index_of_rotation_at_k =
                kpoints.index_of_rotation_at_k()[ik][isym];
            bool is_time_reversal_used_at_k =
                kpoints.is_time_reversal_used_at_k()[ik][isym];
            ofs.write(reinterpret_cast<const char *>(&index_of_rotation_at_k),
                      sizeof(int));
            ofs.write(
                reinterpret_cast<const char *>(&is_time_reversal_used_at_k),
                sizeof(bool));
        }
    }
    ...
    for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
        ofs.write(reinterpret_cast<const char *>(&num_G_at_k_scf[ik]),
                  sizeof(int));
    }
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
            for (int isym = 0; isym < num_sym_at_k[ik]; isym++) {
                Eigen::VectorXi Gindex_at_k_scf =
                    plane_wave_basis.Gindex_at_k_scf()[ispin][ik][isym];
                Eigen::VectorXcd phase_factor_at_k =
                    plane_wave_basis.phase_factor_at_k()[ispin][ik][isym];
                for (int ipw_at_k = 0; ipw_at_k < num_G_at_k_scf[ik];
                     ipw_at_k++) {
                    ofs.write(reinterpret_cast<const char *>(
                                  &Gindex_at_k_scf(ipw_at_k)),
                              sizeof(int));

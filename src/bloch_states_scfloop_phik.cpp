...
void BlochStates::reset_phik(const int &ispin, const int &ik,
    ...
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        &phik_ref_ =
            ...
            : (right_or_left == "left" && calc_mode == "SCF")   ? phik_left_scf_
                                                              : phik_left_band_;
...
void BlochStates::bcast_phik(const bool bcast_phik_left,
    ...
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        &phik_left_ref_ = calc_mode == "SCF" ? phik_left_scf_ : phik_left_band_;

    for (int ispin = 0; ispin < phik_ref_.size(); ispin++) {
        for (int ik = 0; ik < phik_ref_[ispin].size(); ik++) {
            for (int iband = 0; iband < phik_ref_[ispin][ik].size(); iband++) {
                for (int ispinor = 0;
                     ispinor < phik_ref_[ispin][ik][iband].size(); ispinor++) {
                    ...
                    if (bcast_phik_left) {
                        MPI_Bcast(
                            phik_left_ref_[ispin][ik][iband][ispinor].data(),
    ...
    if (calc_mode == "SCF" && mixes_density_matrix) {
        ...
        for (int ispin = 0; ispin < phik_scf_old_.size(); ispin++) {
            for (int ik = 0; ik < phik_scf_old_[ispin].size(); ik++) {
                if (!am_i_mpi_rank0) // allocate before MPI_Bcast
                {
                    ...
                    for (int iband = 0; iband < num_occupied_bands_[ispin][ik];
                        ...
                        for (int ispinor = 0; ispinor < num_spinor; ispinor++) {
                            phik_scf_old_[ispin][ik][iband][ispinor].resize(
                                num_G_at_k);
                        }
                    }

                    if (bcast_phik_left) {
                        phik_left_scf_old_[ispin][ik].resize(
                            num_occupied_bands_[ispin][ik]);
                        phik_left_scf_old_[ispin][ik]
                            .shrink_to_fit(); // memory saving
                        for (int iband = 0;
                             iband < num_occupied_bands_[ispin][ik]; iband++) {
                            phik_left_scf_old_[ispin][ik][iband].resize(
                                num_spinor);
                            for (int ispinor = 0; ispinor < num_spinor;
                                 ispinor++) {
                                phik_left_scf_old_[ispin][ik][iband][ispinor]
                                    .resize(num_G_at_k);
                            }
                        }
                    } // bcast_phik_left
                ...
                for (int iband = 0; iband < phik_scf_old_[ispin][ik].size();
                     iband++) {
                    for (int ispinor = 0;
                         ispinor < phik_scf_old_[ispin][ik][iband].size();
                         ispinor++) {
                        MPI_Bcast(
                            phik_scf_old_[ispin][ik][iband][ispinor].data(),

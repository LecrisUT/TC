...
void calc_hamiltonian::tc2h(
    ...
    // \nabla_1 u_12) = +1.
    //                         But since we consider both u_para and u_anti,
    //                         will be divided with 2 when
    //                         num_independent_spins==1. *Note* density in 2a_h
    //                         and 2b_h1 are already divided with 2 (to say,
    //                         density(up) or density(down)).
        ...
        for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
            for (int isymq = 0; isymq < kpoints.kvectors_scf()[iq].size();
                ...
                for (int ibandq = -nbands_old;
                     ibandq < bloch_states.num_occupied_bands()[ispin][iq];
                     ibandq++) {
                    if (ibandq >= 0) {
                        ...
                        plane_wave_basis.get_orbital_FFTgrid(
                            ...
                            kpoints.is_time_reversal_used_at_k()[iq][isymq],
                            bloch_states
                                .phik_scf_old()[ispin][iq][-1 - ibandq][0],
                            orbital, "SCF");
                    }
                    for (int ipw = 0; ipw < plane_wave_basis.size_FFT_grid();
                         ipw++) {
                        for (int idim = 0; idim < 3; idim++) {
                            grad_orbital[idim](ipw) =
                                (qvect(idim) + Gvect[ipw](idim)) * orbital(ipw);
                        }
                    }
                    if (is_bitc) {
                        if (ibandq >= 0) {
                            plane_wave_basis.get_orbital_FFTgrid(
                                ...
                                kpoints.is_time_reversal_used_at_k()[iq][isymq],
                                bloch_states
                                    .phik_left_scf()[ispin][iq][ibandq][0],
                                orbital, "SCF");
                        } else {
                            plane_wave_basis.get_orbital_FFTgrid(
                                ispin, iq, isymq,
                                kpoints.is_time_reversal_used_at_k()[iq][isymq],
                                bloch_states
                                    .phik_left_scf_old()[ispin][iq][-1 - ibandq]
                                                        [0],
                                orbital, "SCF");
                        }
                    }
                    plane_wave_basis.FFT_backward(orbital,
                                                  orbital); // -> phi_q(R)
                    double filq =
                        ibandq >= 0
                            ? bloch_states.filling()[ispin][iq][ibandq]
                            : bloch_states
                                  .filling_old()[ispin][iq][-1 - ibandq];
                    orbital = filq * orbital.conjugate();
                    for (int idim = 0; idim < 3; idim++) {
                        plane_wave_basis.FFT_backward(grad_orbital[idim],
                                                      grad_orbital[idim]);
                        phiq_gphiq[ispin][idim] =
                            phiq_gphiq[ispin][idim].array() +
                            orbital.array() *
                                grad_orbital[idim].array(); // <q|\nabla|q>
                    }
                } // ibandq
            }     // isymq
        }         // iq
        for (int idim = 0; idim < 3; idim++) {
            plane_wave_basis.FFT_forward(phiq_gphiq[ispin][idim],
                                         phiq_gphiq[ispin][idim]); // -> (G)
        }
    } // ispin

    // [2b_h1] dnu = sum_q <*,q| \nabla_1 u_12 | *,q>
    std::vector<std::vector<Eigen::VectorXcd>> dnu(
        num_independent_spins); // spin index of x1
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        dnu[ispin].resize(3);
        for (int idim = 0; idim < 3; idim++) {
            dnu[ispin][idim] =
    ...
    double spin_factor =
        num_independent_spins == 1
            ? 2.0
            : 1.0; // canceled with filling=2.0 for num_independent_spins==1
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        for (int jspin = 0; jspin < 2; jspin++) {
            int jspin_ref = num_independent_spins == 1 ? 0 : jspin;

            orbital =
                bloch_states.density()[jspin]; // density(R). Name "orbital" is
                                               // meaningless here
            ...
            for (int ipw = 0; ipw < plane_wave_basis.size_FFT_grid(); ipw++) {
                ...
                V_2body[ispin](ipw) +=
                    potentials.jastrow.tc_2body(Gvect[ipw], ispin, jspin) *
                    orbital(ipw);
                ...
                for (int idim = 0; idim < 3; idim++) {
                    ...
                    // [2b_h2] sum_q <*,q| \nabla_2 u_21 \nabla_2 |*,q>
                    V_2body[ispin](ipw) += uk * Gvect[ipw](idim) *
                                           phiq_gphiq[jspin_ref][idim](ipw) /
                                           spin_factor;
        ...
        plane_wave_basis.FFT_backward(V_2body[ispin],
                                      V_2body[ispin]); // -> V_2body(R)
        for (int idim = 0; idim < 3; idim++) {
            plane_wave_basis.FFT_backward(dnu[ispin][idim],
                                          dnu[ispin][idim]); // -> dnu(R)
        }
    }
            ...
            for (int jband = 0; jband < H2phi[ispin][ik].size(); jband++) {
                ...
                for (int jspinor = 0; jspinor < num_spinor; jspinor++) {
                    ...
                    for (int ipw = 0; ipw < plane_wave_basis.size_FFT_grid();
                        ...
                        for (int idim = 0; idim < 3; idim++) {
                            grad_orbital[idim](ipw) =
                                (kvect(idim) + Gvect[ipw](idim)) * orbital(ipw);
                        }
                    }
                    for (int idim = 0; idim < 3; idim++) {
                        plane_wave_basis.FFT_backward(grad_orbital[idim],
                    ...
                    for (int idim = 0; idim < 3; idim++) {
                        V_orbital = V_orbital.array() -
                                    grad_orbital[idim].array() *
                                        dnu[ispin][idim].array(); // I^2 = -1
                    }

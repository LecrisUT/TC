...
void CrystalStructure::set(
    const Eigen::Matrix3d &lattice_vectors,
    const Eigen::Matrix3d &reciprocal_vectors, const int &num_atomic_species,
...
void CrystalStructure::bcast(const bool am_i_mpi_rank0) {
    ...
    for (int iatom = 0; iatom < num_atoms_; iatom++) {
        MPI_Bcast(atomic_position_cartesian_[iatom].data(), 3, MPI_DOUBLE, 0,
                  MPI_COMM_WORLD);
    }
}

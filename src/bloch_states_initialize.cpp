...
void BlochStates::set_num_bands_qe(const std::vector<int> &num_bands_qe,
    ...
    } else                             // check consistency
    {
        ...
        num_bands_tc_.assign(num_independent_spins,
                             nbtmp); // change the size of the array and filled
                                     // with num_bands_tc_[0]
    }
}

// called in set_qe_xml(). also set eigenvalues_scf/band = eigenvalues_qe
void BlochStates::set_eigenvalues_qe(
    const std::vector<std::vector<double>> &eigenvalues,
    const std::string &calc_mode) {
    const int num_irreducible_kpoints = eigenvalues.size();
    const int num_independent_spins = num_bands_qe_.size();
    const int num_bands_qe_total = num_independent_spins == 2
                                       ? num_bands_qe_[0] + num_bands_qe_[1]
                                       : num_bands_qe_[0];
    ...
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        ...
        for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
            ...
            eigenvalues_tc_[ispin][ik].resize(
                num_bands_tc_[ispin]); // Note: num_band_tc_ (not num_bands_qe_)
            ...
            for (int iband = 0; iband < num_bands_qe_[ispin]; iband++) {
                ...
                if (iband < num_bands_tc_[ispin]) {
                    eigenvalues_tc_[ispin][ik][iband] =
                        eigenvalues_qe_[ispin][ik][iband];
                }
            }
        }
    }
...
void BlochStates::resize_filling(const int &num_independent_spins,
    ...
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        ...
        for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
            filling_old_[ispin][ik].resize(
                0); // resized in the SCF loop. Only occupied bands are
                    // allocated for filling_old_.
        }
    }
}

void BlochStates::resize_num_occupied_bands(
    const int &num_independent_spins, const int &num_irreducible_kpoints_scf) {
    assert(num_independent_spins > 0);
    assert(num_irreducible_kpoints_scf > 0);
...
// In the SCF calculation, this function is called once when reading input.in
// with the "SCF" switch. In the BAND calculation, this function is called
// twice.
//   First call = "BAND" switch when reading input.in to initialize
//   num_bands_band Second call = "SCF" switch when reading tc_scfinfo.dat to
//   initialize num_bands_scf
// For both cases, the first call is skipped when num_bands_tc is not specified
// in input.in. In that case, num_bands_tc = num_bands_qe is used as a default
// setting in set_num_bands_qe().
void BlochStates::set_num_bands_tc(const std::vector<int> &num_bands_tc,
    ...
    for (auto nb : num_bands_tc) {
        if (nb <= 0) {
            error_messages::inappropriate_argument("num_bands_tc", nb,
                                                   "should be positive");
        }
    }
...
void BlochStates::set_qe_xml(
    const std::vector<int> &num_bands_qe,
    const std::vector<std::vector<double>> &eigenvalues,
    const std::string &calc_mode, const double &num_electrons) {
    assert(calc_mode == "SCF" || calc_mode == "BAND");
...
// phik[spin][k-point][band][num_spinor][npw_at_k]: wave function in G-space at
// each irreducible k-point
void BlochStates::resize_phik_qe(const PlaneWaveBasis &plane_wave_basis,
                                 const bool is_spinor,
                                 const int &num_independent_spins,
                                 const std::string &calc_mode) {
    assert(calc_mode == "SCF" || calc_mode == "BAND");
    assert(num_independent_spins > 0);

    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        &phik_ref_ = calc_mode == "SCF" ? phik_scf_ : phik_band_;
    const std::vector<int> &num_G_at_k =
        calc_mode == "SCF" ? plane_wave_basis.num_G_at_k_scf()
                           : plane_wave_basis.num_G_at_k_band();
    const std::vector<int> &num_bands_tc_ =
        calc_mode == "SCF" ? num_bands_scf_ : num_bands_band_;
        ...
        for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
            ...
            for (int iband = 0; iband < num_bands_tc_[ispin]; iband++) {
                ...
                for (int ispinor = 0; ispinor < num_spinor; ispinor++) {
                    ...
                    // phik_ref_[ispin][ik][iband][ispinor].shrink_to_fit(); //
                    // memory saving
                }
            }
        }
...
void BlochStates::set_phik_qe(const int ispin, const int ik,
    ...
    for (int iband = 0; iband < num_bands_tc_[ispin]; iband++) {
        ...
        for (int ispinor = 0; ispinor < num_spinor; ispinor++) {
            ...
            for (int ipw_at_k = 0; ipw_at_k < npw_at_k; ipw_at_k++) {
                phik_ref_[ispin][ik][iband][ispinor](ipw_at_k) =
                    evc[icount]; // evc(npol*igwx, nbnd[is]) in fortran
...
void BlochStates::bcast_qe_input(const PlaneWaveBasis &plane_wave_basis,
    ...
    if (am_i_mpi_rank0) {
        num_irreducible_kpoints = eigenvalues_qe_[0].size();
    }
    ...
    // num_bands_tc & num_bands_qe ... the former can be specified by input.in
    // or initialized by read_qe(). Thus, Bcast is performed here.
    ...
    if (!am_i_mpi_rank0) {
        set_num_bands_tc(ivec, calc_mode);
    }
    if (am_i_mpi_rank0) {
        ivec = num_bands_qe_;
    }
    MPI_Bcast(&ivec[0], ivec.size(), MPI_INT, 0, MPI_COMM_WORLD);
    if (!am_i_mpi_rank0) {
        set_num_bands_qe(ivec, calc_mode);
    }
    ...
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        ...
        for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
            ...
            if (!am_i_mpi_rank0) {
                eigenvalues_tc_[ispin][ik].resize(num_bands_tc_[ispin]);
            }
            MPI_Bcast(&eigenvalues_qe_[ispin][ik][0],
                      eigenvalues_qe_[ispin][ik].size(), MPI_DOUBLE, 0,
                      MPI_COMM_WORLD);
            MPI_Bcast(&eigenvalues_tc_[ispin][ik][0],
                      eigenvalues_tc_[ispin][ik].size(), MPI_CXX_DOUBLE_COMPLEX,
                      0, MPI_COMM_WORLD);
        }
    }
    ...
    // phik
    if (!am_i_mpi_rank0) {
        resize_phik_qe(plane_wave_basis, is_spinor, num_independent_spins,
                       calc_mode);
    }
    bcast_phik(false, false, calc_mode,
               am_i_mpi_rank0); // false(1st): do not bcast phik_left.
                                // false(2nd): do not bcast phik_scf_old
    if (calc_method == "BITC" && calc_mode == "SCF") // phik_left
    {
        phik_left_scf_ = phik_scf_;
        phik_left_scf_old_ = phik_scf_old_;
    }
    if (calc_method == "BITC" && calc_mode == "BAND") {
        phik_left_band_ = phik_band_;
    } // phik_left

    // not bcast but required initialization
    // In BAND calculation, resize_filling & resize_num_occupied_bands will be
    // called in bcast_scfinfo().
    if (calc_mode == "SCF") {
        resize_filling(num_independent_spins, num_irreducible_kpoints);
        resize_num_occupied_bands(num_independent_spins,
...
void BlochStates::bcast_scfinfo(const PlaneWaveBasis &plane_wave_basis,
    ...
    if (am_i_mpi_rank0) {
        num_irreducible_kpoints_scf = plane_wave_basis.num_G_at_k_scf().size();
    }
    MPI_Bcast(&num_irreducible_kpoints_scf, 1, MPI_INT, 0, MPI_COMM_WORLD);

    int num_independent_spins;
    if (am_i_mpi_rank0) {
        num_independent_spins = num_bands_scf_.size();
    }
    MPI_Bcast(&num_independent_spins, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // num_bands_scf_
    if (!am_i_mpi_rank0) {
        num_bands_scf_.resize(num_independent_spins);
    }
    MPI_Bcast(&num_bands_scf_[0], num_independent_spins, MPI_INT, 0,
              MPI_COMM_WORLD);
...
void BlochStates::set_phik_from_tc_wfc(const int &ispin, const int &ik,
                                       const int &iband,
...
void BlochStates::set_eigenvalues_from_tc_energy(
    const int &ispin, const int &ik, const Eigen::VectorXcd &eigenvalues_temp,
    const std::string &calc_mode) {
    assert(calc_mode == "SCF" || calc_mode == "BAND");
    std::vector<std::vector<std::vector<Complex>>> &eigenvalues_ref_ =
        calc_mode == "SCF" ? eigenvalues_scf_ : eigenvalues_band_;
    const std::vector<int> num_bands_ref_ =
        calc_mode == "SCF" ? num_bands_scf_ : num_bands_band_;

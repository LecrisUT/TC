...
// Simpson's rule integration on the pseudo-potential radial mesh. Integrate
// "function".
double Potentials::simpson(const std::vector<double> &function,
    ...
    for (int imesh = 0; imesh < mesh_size - 2; imesh = imesh + 2) {
        ...
        sum += function[imesh + 1] *
               pseudo_weight_rmesh_[iatomic_species][imesh + 1] * 4;
        sum += function[imesh + 2] *
               pseudo_weight_rmesh_[iatomic_species][imesh + 2];
    }
    return sum / 3.0;
}

// Rydberg -> Hartree
// (also required for pseudo_nlcc when TC++ supports non-linear core
// correction...)
void Potentials::set_unit(std::vector<std::vector<double>> &pseudo_local,
    ...
    for (int iatomic_species = 0; iatomic_species < num_atomic_species;
        ...
        assert(pseudo_dij[iatomic_species].rows() ==
               pseudo_dij[iatomic_species].cols());
        for (int imesh = 0; imesh < mesh_size; imesh++) {
            pseudo_local[iatomic_species][imesh] /= 2.0;
        }
        pseudo_dij[iatomic_species] /= 2.0;
    }
}

void Potentials::set_Vpp_local(
    ...
    for (int ipw = 0; ipw < npw; ipw++) {
        ...
        Gnorm(ipw) =
            std::sqrt(Gvector.row(ipw) * Gvector.row(ipw).transpose()); // |G|
    }
    ...
    const double param_ewald =
        PI /
        std::cbrt(crystal_structure.unit_cell_volume()); // cbrt = cubic root

    // Ewald sum (usual): Ewald parameter = a,
    //   \sum_R 1/|r-R| = \sum_R erfc(a|r-R|)/|r-R| + 4pi/V \sum_G
    //   exp(-G^2/(4a^2))/(G^2) exp(iGr)
    // Formula used here, which can be obtained from the above equation: (Note!
    // erfc = 1 - erf)
    //   \sum_R erf(a|r-R|)/|r-R| = 4pi/V \sum_G exp(-G^2/(4a^2))/(G^2) exp(iGr)
    // Below we call left-hand-side "term 1" and right-hand-side "term 2",
    // respectively.
    ...
    // (2) perform Fourier-Transform of Vpp_short (here, summation over
    // lattice-periodicity is performed) (3) add "term 2" to compensate (1).
    //     At the same time, G=0 of -Z/r is subtracted, which should be canceled
    //     with G=0 of el-el + nucl-nucl.
    for (int iatomic_species = 0; iatomic_species < num_atomic_species;
         iatomic_species++) {
        int mesh_size = pseudo_rmesh_[iatomic_species].size();
        local_temp.resize(mesh_size);
        local_temp2.resize(mesh_size);
        // (1) subtract "term 1" in R-space = subtract (-Z/r)*.... = add
        // (Z/r)*...
        for (int imesh = 0; imesh < mesh_size;
             imesh++) // radial mesh in pseudopot.
        {
            if (pseudo_rmesh_[iatomic_species][imesh] > 1e-8) {
                local_temp[imesh] =
                    pseudo_local[iatomic_species][imesh] +
                    (Z_valence[iatomic_species] /
                     pseudo_rmesh_[iatomic_species][imesh]) *
                        std::erf(param_ewald *
                                 pseudo_rmesh_[iatomic_species][imesh]);
            } else // erf(ax)/x = a*(erf(ax)/(ax)) -> 2a/sqrt(PI) for x->0
            {
                local_temp[imesh] = pseudo_local[iatomic_species][imesh] +
                                    Z_valence[iatomic_species] * 2.0 *
        ...
        for (int ipw = 0; ipw < npw; ipw++) {
            ...
            // (1/V) \int pseudo_local(r) exp(-iGr) d^3 r = (4pi/V) \int
            // pseudo_local(r) (sin(|G|r)/|G|r) r^2 dr
            Vpp_local_atom_G[iatomic_species](ipw) =
                simpson(local_temp2, iatomic_species);

            // (3) add "term2" in G-space
            if (ipw != 0) {
                Vpp_local_atom_G[iatomic_species](ipw) -=
                    Z_valence[iatomic_species] *
                    std::exp(-Gnorm(ipw) * Gnorm(ipw) /
                             (4.0 * param_ewald * param_ewald)) /
                    (Gnorm(ipw) * Gnorm(ipw));
            } else {
                // Note! we consider G=0 limit of [exp(-G^2/(4*param^2)) -
                // 1]/(G^2) -> -1/(4*param^2), instead of
                // exp(-G^2/(4*param^2))/(G^2). This is because G=0 of -Z/r
                // should be canceled with G=0 of el-el + nucl-nucl.
                Vpp_local_atom_G[iatomic_species](ipw) +=
                    Z_valence[iatomic_species] /
                    (4.0 * param_ewald * param_ewald);
            }
            Vpp_local_atom_G[iatomic_species](ipw) *=
                (FourPI / unit_cell_volume);
        } // ipw
    }     // iatomic_species

    // set Vpp_local_G using Vpp_local_atom_G
    // Vpp_local(r) = \sum_{iatom} Vpp_local_atom(r-r_{iatom})
    // where FT[Vpp_local_atom(r-r_{iatom})] (G) = FT[Vpp_local_atom(r)] (G) *
    // exp^{-iGr_{iatom}}
    ...
    for (int iatom = 0; iatom < num_atoms; iatom++) {
        ...
        Eigen::VectorXd GdotR =
            Gvector *
            crystal_structure.atomic_position_cartesian()[iatom]; // array(npw)
        for (int ipw = 0; ipw < npw; ipw++) {
            Vpp_local_G(ipw) +=
                (std::cos(GdotR(ipw)) - I * std::sin(GdotR(ipw))) *
                Vpp_local_atom_G[iatomic_species](ipw);
        } // np
    }     // iatom
    plane_wave_basis.FFT_backward(Vpp_local_G,
                                  Vpp_local_G); // G-space to R-space

    Vpp_local_.resize(npw);
    for (int ipw = 0; ipw < npw; ipw++) {
        Vpp_local_[ipw] = Vpp_local_G[ipw].real();
    } // take a real part
}

// public

void Potentials::set_upf(
    const std::vector<double> &Z_valence,
...
void Potentials::bcast_upf(const bool am_i_mpi_rank0) {
    ...
    for (int iatomic_species = 0; iatomic_species < num_atomic_species;
        ...
        MPI_Bcast(&pseudo_rmesh_[iatomic_species][0], mesh_size, MPI_DOUBLE, 0,
                  MPI_COMM_WORLD);
        MPI_Bcast(&pseudo_weight_rmesh_[iatomic_species][0], mesh_size,
                  MPI_DOUBLE, 0, MPI_COMM_WORLD);
        for (int iproj = 0; iproj < num_projectors; iproj++) {
            MPI_Bcast(&pseudo_beta_[iatomic_species][iproj][0], mesh_size,
                      MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }

        MPI_Bcast(&pseudo_lbeta_[iatomic_species][0], num_projectors, MPI_INT,
                  0, MPI_COMM_WORLD);
        MPI_Bcast(pseudo_dij_[iatomic_species].data(),
                  num_projectors * num_projectors, MPI_DOUBLE, 0,
                  MPI_COMM_WORLD);
    }
}
...
//   [Note: For k+G=0, j_0(0) = 1, Y_00 = 1/sqrt(4pi) have non-zero
//   contribution. (j_l(0)=0 for l!=0)]
...
// = exp(i(k+G)R_at) i^l Y_lm(e_{k+G})^* [4pi/sqrt(V) \int dr r^2 beta^*(r)
// j_l(|k+G|r)]
//
// In projector_nonlocal(), exp(i(k+G)R_at) is not multiplied.
std::vector<Eigen::VectorXcd> Potentials::projector_nonlocal(
    const CrystalStructure &crystal_structure,
    const PlaneWaveBasis &plane_wave_basis, const Eigen::VectorXi &Gindex_at_k,
    const Eigen::Vector3d &kvector,
    const std::vector<std::vector<Eigen::VectorXcd>> &ylm, const int &ispin,
    const int &ik, const int &iatomic_species, const int &iproj) const {
    ...
    if (lbeta >= ylm.size()) {
        error_messages::stop("lbeta too large: need to modify the code.");
    }
    // m = 0, 1, ..., l. For negative m, we just use (Y_lm)^*. (-1)^m does not
    // matter when pseudo_dij is diagonal w.r.t. l.
    assert(ylm[lbeta].size() == lbeta + 1);

    // m = -l, -l+1, ..., l (2*lbeta +1 indices)
    std::vector<Eigen::VectorXcd> return_vector(2 * lbeta +
                                                1); // return this array
    for (int im = 0; im < 2 * lbeta + 1; im++) {
        return_vector[im].resize(num_G_at_k);
    }
    ...
    const double factor =
        FourPI / std::sqrt(crystal_structure.unit_cell_volume());
    Complex cfactor = 1.0;
    for (int il = 0; il < lbeta; il++) {
        cfactor *= I;
    } // I^l
    ...
    for (int ipw_at_k = 0; ipw_at_k < num_G_at_k; ipw_at_k++) {
        Eigen::Vector3d kGvect =
            ...
            (kvector + plane_wave_basis.get_Gvector(Gindex_at_k(ipw_at_k))
                           .cast<double>());
        double kG = kGvect.norm();

        // calculate beta_jl(|k+G|) = [4pi/sqrt(V) \int dr r^2 beta^*(r)
        // j_l(|k+G|r)].
        for (int imesh = 0; imesh < mesh_size;
             imesh++) // radial mesh in pseudopot.
        {
            ...
            temp_rfunc[imesh] =
                rmesh *
                pseudo_beta_[iatomic_species][iproj]
                            [imesh] // beta in UPF = r*beta (Length^{-1/2})
                * boost::math::sph_bessel(lbeta, kG * rmesh);
            //            temp_rfunc[imesh] =
            //            rmesh*rmesh*pseudo_beta_[iatomic_species][iproj][imesh]
            //                * boost::math::sph_bessel(lbeta, kG*rmesh);
        } // imesh
        beta_jl = factor * simpson(temp_rfunc, iatomic_species);

        // calculate i^l Y_lm(e_{k+G})^* beta_jl(|k+G|)
        return_vector[lbeta](ipw_at_k) =
            cfactor * ylm[lbeta][0](ipw_at_k) * beta_jl; // im=0
        for (int im = 1; im < lbeta + 1; im++) {
            return_vector[lbeta - im](ipw_at_k) =
                cfactor * std::conj(ylm[lbeta][im](ipw_at_k)) * beta_jl;
            return_vector[lbeta + im](ipw_at_k) =
                cfactor * ylm[lbeta][im](ipw_at_k) * beta_jl;
        }
    }

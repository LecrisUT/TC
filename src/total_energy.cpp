...
void TotalEnergy::set_ewald_energy(const bool is_heg,
                                   const double &ewald_energy) {
    if (!is_heg) {
        ewald_energy_ = ewald_energy;
    } // ewald_energy = 0 for is_heg==true
}
...
void TotalEnergy::bcast_qe_input(const int &num_independent_spins,
    ...
    if (calc_mode == "SCF") {
        resize_energy(num_independent_spins, num_irreducible_kpoints,
                      num_bands_scf, calc_method);
    }
    if (!is_heg) {
...
void TotalEnergy::reset_energies(const int &ispin, const int &ik,
    ...
    for (int iband = 0; iband < num_bands_tc; iband++) {
        ...
        if (uses_3body) {
            energy_3body_[ispin][ik][iband] = energy_3body(iband, iband);
        }
    }
}
...
void TotalEnergy::bcast_energies(const bool uses_3body) {
    for (int ispin = 0; ispin < energy_1body_.size(); ispin++) {
        for (int ik = 0; ik < energy_1body_[ispin].size(); ik++) {
            MPI_Bcast(&energy_1body_[ispin][ik][0],
                      energy_1body_[ispin][ik].size(), MPI_CXX_DOUBLE_COMPLEX,
                      0, MPI_COMM_WORLD);
            MPI_Bcast(&energy_2body_[ispin][ik][0],
                      energy_2body_[ispin][ik].size(), MPI_CXX_DOUBLE_COMPLEX,
                      0, MPI_COMM_WORLD);
            if (uses_3body) {
                MPI_Bcast(&energy_3body_[ispin][ik][0],
                          energy_3body_[ispin][ik].size(),
...
void TotalEnergy::calc_total_energy(
    ...
    const double &fermi_energy, const bool uses_3body,
    ...
    for (int ispin = 0; ispin < num_independent_spins; ispin++) {
        for (int ik = 0; ik < num_irreducible_kpoints_scf; ik++) {
            if (kpoints.kweight_scf()[ik] < 1e-8) {
                continue;
            } // zero k-weight (e.g. band k-point)
            ...
            for (int isym = 0; isym < kpoints.kvectors_scf()[ik].size();
                ...
                if (!kpoints.is_time_reversal_used_at_k()[ik][isym]) {
                    for (int iband = 0; iband < num_bands_tc; iband++) {
                        total_energy_1body_ += filling[ispin][ik][iband] *
                                               energy_1body_[ispin][ik][iband];
                        total_energy_2body_ += filling[ispin][ik][iband] *
                                               energy_2body_[ispin][ik][iband] /
                                               2.0;
                        if (uses_3body) {
                            total_energy_3body_ +=
                                filling[ispin][ik][iband] *
                                energy_3body_[ispin][ik][iband] / 3.0;
                        }

                        double filling_sigma0 =
                            eigenvalues_scf[ispin][ik][iband].real() <
                                    fermi_energy + 1e-8
                                ? spin_factor
                                : 0.0; // fully-occupied or empty
                        total_energy_sigma0 +=
                            filling_sigma0 *
                            (energy_1body_[ispin][ik][iband] +
                    ...
                    for (int iband = 0; iband < num_bands_tc; iband++) {
                        total_energy_1body_ +=
                            filling[ispin][ik][iband] *
                            std::conj(energy_1body_[ispin][ik][iband]);
                        total_energy_2body_ +=
                            filling[ispin][ik][iband] *
                        ...
                        if (uses_3body) {
                            total_energy_3body_ +=
                                filling[ispin][ik][iband] *
                                std::conj(energy_3body_[ispin][ik][iband]) /
                                3.0;
                        }
    ...
    total_energy_difference_ = ewald_energy_ + total_energy_1body_ +
                               total_energy_2body_ + total_energy_3body_ -
                               total_energy_;
    total_energy_ = ewald_energy_ + total_energy_1body_ + total_energy_2body_ +
                    total_energy_3body_;

    if (am_i_mpi_rank0) {
        *ost << "   Total energy (Ewald energy) = " << ewald_energy_ << " Ht."
             << std::endl;
        *ost << "   Total energy (1-body terms) = ( "
             << total_energy_1body_.real() << " , "
             << total_energy_1body_.imag() << " ) Ht." << std::endl;
        *ost << "   Total energy (2-body terms) = ( "
             << total_energy_2body_.real() << " , "
             << total_energy_2body_.imag() << " ) Ht." << std::endl;
        if (uses_3body) {
            *ost << "   Total energy (3-body terms) = ( "
                 << total_energy_3body_.real() << " , "
                 << total_energy_3body_.imag() << " ) Ht." << std::endl;
        }
        *ost << "   Total energy = ( " << total_energy_.real() << " , "
             << total_energy_.imag() << " ) Ht." << std::endl;
        if (kpoints.smearing_mode() == "gaussian") {
            *ost << "   Total energy (sigma->0) = ( "
                 << ((total_energy_ + total_energy_sigma0) / 2.0).real()
                 << " , "
                 << ((total_energy_ + total_energy_sigma0) / 2.0).imag()
                 << " ) Ht." << std::endl;
        }
        *ost << "   Total energy difference from the previous loop = ( "
             << total_energy_difference_.real() << " , "

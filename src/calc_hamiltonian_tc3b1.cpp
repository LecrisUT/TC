...
void calc_hamiltonian::tc3b1(
    ...
    const double three_body_factor =
    ...
    std::vector<std::vector<Eigen::VectorXcd>> dnu(
        num_independent_spins); // spin index of x2
    ...
    std::vector<Eigen::VectorXcd> V_3body(
    ...
    for (int ispin2 = 0; ispin2 < num_independent_spins;
         ispin2++) // spin index of x2
    {
        ...
        for (int idim = 0; idim < 3; idim++) {
            plane_wave_basis.FFT_backward(dnu[ispin2][idim],
                                          dnu[ispin2][idim]); // -> dnu(R)
        }

        // set sum_q1 sum_q2 <*,q1,q2| \nabla_2 u_23 |*,q1,q2>
        orbital = bloch_states.density()[ispin2]; // density(R). Name "orbital"
                                                  // is meaningless here
        for (int idim = 0; idim < 3; idim++) {
            dnu[ispin2][idim] = dnu[ispin2][idim].array() * orbital.array();
            plane_wave_basis.FFT_forward(dnu[ispin2][idim],
                                         dnu[ispin2][idim]); // -> (G)
        }
    }
    ...
    // *,q1,q2>
    for (int ispin1 = 0; ispin1 < num_independent_spins;
         ispin1++) // spin index of x1
    {
        ...
        plane_wave_basis.FFT_backward(V_3body[ispin1],
                                      V_3body[ispin1]); // -> (R)
    }

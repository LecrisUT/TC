...
bool is_matched(const Eigen::Vector3d &vec,
                const std::vector<Eigen::Vector3d> &vec_list) {
    double diff, norm;
    for (auto v : vec_list) {
        norm = 0.0;
...
void Kpoints::set_kvectors_by_symmetry(const Spin &spin,
    ...
    for (int ik = 0; ik < num_irreducible_kpoints_scf_; ik++) {
        ...
        for (int isym = 0; isym < symmetry.num_symmetry_operations(); isym++) {
            ...
            vec_tmp = symmetry.rotation()[isym].cast<double>().transpose() *
                      kvectors_scf_[ik][0]; // 0 = irreducible k-point
            for (int idim = 0; idim < 3; idim++) {
                if (std::abs(vec_tmp(idim)) < 1e-4) {
                    vec_tmp(idim) = 0.0;
                }
            } // almost 0
            ...
            // k' -> -k'
            if (symmetry.is_time_reversal_symmetric() &&
                !is_matched(-vec_tmp, vec_list)) {
                vec_list.push_back(-vec_tmp);
                kvectors_scf_[ik].push_back(-vec_tmp);
    ...
    for (int ik = 0; ik < num_irreducible_kpoints_scf_; ik++) {
        if (kweight_scf_[ik] > 1e-8) {
            num_kpoints_ += kvectors_scf_[ik].size();
        } // non-zero k-weight
    }

    // check consistency with k_weight
    double fact = (spin.num_independent_spins() == 1 && !spin.is_spinor())
                      ? 2.0
                      : 1.0; // 2 for no-spin
    fact /= static_cast<double>(num_kpoints_);
    for (int ik = 0; ik < num_irreducible_kpoints_scf_; ik++) {
        if ((kweight_scf_[ik] > 1e-6) &&
            (std::abs(fact * kvectors_scf_[ik].size() - kweight_scf_[ik]) >
             1e-6)) {
            std::cout << "ik: " << ik << "  kweight in QE: " << kweight_scf_[ik]
                      << ", kweight in TC: " << fact * kvectors_scf_[ik].size()
                      << std::endl;
            error_messages::stop(
                "Error in k-weight. (e.g., a k-mesh that breaks a space-group "
                "symmetry is not applicable)");
        }
    }
    ...
    *ost << "  Print symmetrically-repiclated kvectors:" << std::endl;
    ...
    for (int ik = 0; ik < num_irreducible_kpoints_scf_; ik++) {
        *ost << "  Irreducible k-vector (ik = " << ik << ") "
             << kvectors_scf_[ik][0](0) << " " << kvectors_scf_[ik][0](1) << " "
        ...
        for (int isymk = 0; isymk < kvectors_scf_[ik].size(); isymk++) {
            ...
            *ost << "   transformation operator (in k-space, applied to the "
                    "irreducible k. Transpose of the symmorphic operator in "
                    "r-space) "
                 << std::endl;
            *ost << symmetry.rotation()[index_of_rotation_at_k_[ik][isymk]]
                        .transpose()
                 << std::endl;
            *ost << "   time-reversal symmetry = ";
            if (is_time_reversal_used_at_k_[ik][isymk]) {
                *ost << "true" << std::endl;
            } else {
                *ost << "false" << std::endl;
            }
            *ost << "   translation vector (while not applied to the k-vector) "
                    "= ";
            *ost << symmetry.translation()[index_of_rotation_at_k_[ik][isymk]]
                        .transpose()
                 << std::endl;
            *ost << std::endl;
        }
...
void Kpoints::set_tc_input(const std::string &smearing_mode,
                           const double &smearing_width) {
    const std::vector<std::string> list_smearing_mode{"fixed", "gaussian"};
    error_messages::compare_list(
        "smearing_mode", smearing_mode,
        list_smearing_mode); // stop the program if not included in
                             // list_smearing_mode
    smearing_mode_ = smearing_mode;

    if (smearing_width_ > -1e-8) {
        smearing_width_ = smearing_width;
    } // negative value will be discarded
}
...
void Kpoints::bcast_tc_input(const bool am_i_mpi_rank0) {
    ...
    MPI_Bcast(const_cast<char *>(s.data()), length, MPI_CHAR, 0,
              MPI_COMM_WORLD);
    if (!am_i_mpi_rank0) {
        smearing_mode_ = s;
    }
...
void Kpoints::set_qe_input(const Spin &spin, const Symmetry &symmetry,
                           ...
                           const std::string &calc_mode, std::ostream *ost) {
    ...
    if (calc_mode == "SCF") {
        ...
        for (int ik = 0; ik < kvectors_irred.size(); ik++) {
            ...
            for (int idim = 0; idim < 3; idim++) {
                if (std::abs(kvectors_scf_[ik][0](idim)) < 1e-6) {
                    kvectors_scf_[ik][0](idim) = 0.0;
                } // almost 0
            }
        }
        ...
        for (int ik = 0; ik < kvectors_irred.size(); ik++) {
            ...
            for (int idim = 0; idim < 3; idim++) {
                if (std::abs(kvectors_band_[ik][0](idim)) < 1e-6) {
                    kvectors_band_[ik][0](idim) = 0.0;
                } // almost 0
            }
        }
    }
...
void Kpoints::bcast_qe_input(const std::string &calc_mode,
    ...
    if (calc_mode == "SCF") {
        ...
        if (!am_i_mpi_rank0) {
            kvectors_scf_.resize(num_irreducible_kpoints_scf_);
        }
        for (int ik = 0; ik < kvectors_scf_.size(); ik++) {
            int i = kvectors_scf_[ik].size();
            MPI_Bcast(&i, 1, MPI_INT, 0, MPI_COMM_WORLD);
            if (!am_i_mpi_rank0) {
                kvectors_scf_[ik].resize(i);
            }

            for (int isym = 0; isym < i; isym++) {
                MPI_Bcast(kvectors_scf_[ik][isym].data(), 3, MPI_DOUBLE, 0,
                          MPI_COMM_WORLD);
            }
        }

        if (!am_i_mpi_rank0) {
            kweight_scf_.resize(num_irreducible_kpoints_scf_);
        }
        MPI_Bcast(&kweight_scf_[0], kweight_scf_.size(), MPI_DOUBLE, 0,
                  MPI_COMM_WORLD);
        ...
        if (!am_i_mpi_rank0) {
            ...
            for (int ik = 0; ik < kvectors_scf_.size(); ik++) {
                ...
                is_time_reversal_used_at_k_[ik].resize(
                    kvectors_scf_[ik].size());
            }
        }
        std::vector<int> ivec;
        for (int ik = 0; ik < kvectors_scf_.size(); ik++) {
            int nsym = kvectors_scf_[ik].size();
            MPI_Bcast(&index_of_rotation_at_k_[ik][0], nsym, MPI_INT, 0,
                      MPI_COMM_WORLD);
            ...
            if (!am_i_mpi_rank0) {
                for (int is = 0; is < nsym; is++) {
                    is_time_reversal_used_at_k_[ik][is] =
                        ivec[is] == 1 ? true : false;
                }
            }
        }
    } else if (calc_mode == "BAND") {
        MPI_Bcast(&num_irreducible_kpoints_band_, 1, MPI_INT, 0,
                  MPI_COMM_WORLD);
        if (!am_i_mpi_rank0) {
            kvectors_band_.resize(num_irreducible_kpoints_band_);
        }
        for (int ik = 0; ik < kvectors_band_.size(); ik++) {
            if (!am_i_mpi_rank0) {
                kvectors_band_[ik].resize(1);
            }
            MPI_Bcast(kvectors_band_[ik][0].data(), 3, MPI_DOUBLE, 0,
                      MPI_COMM_WORLD);
        }
    }
}

void Kpoints::set_scfinfo(
    const std::vector<double> &kweight_scf,
...
double Kpoints::return_band_filling(const Spin &spin,
                                    const double &orbital_energy,
                                    const double &fermi_energy,
                                    const int &num_electrons,
                                    const int &iband) const {
    const double spin_factor =
        (spin.num_independent_spins() == 1 && !spin.is_spinor())
            ? 2.0
            : 1.0;                 // 2 for no-spin
    if (smearing_mode_ == "fixed") // fermi_energy is not used
    {
        ...
        if (spin.is_spinor()) {
            ...
            if (std::abs(num_occupied_bands - num_electrons) > 1e-5) {
                error_messages::stop("fixed smearing cannot be used for this "
                                     "num_electrons in return_band_filling()");
            }
        } else {
    ...
    } else if (smearing_mode_ ==
               "gaussian") // num_electrons and iband are not used
    {

...
void BlochStates::print_band_energies(const Kpoints &kpoints,
    ...
    for (int ispin = 0; ispin < eigenvalues_band_.size(); ispin++) {
        *ost << "   Spin " << ispin << std::endl;
        for (int ik = 0; ik < eigenvalues_band_[ispin].size(); ik++) {
            *ost << "   k-vector " << kpoints.kvectors_band()[ik][0].transpose()
                 << std::endl;
            *ost << "   (band index, energy (eV))" << std::endl;
            for (int iband = 0; iband < eigenvalues_band_[ispin][ik].size();
                 iband++) {
                *ost << "   " << iband << " "
                     << eigenvalues_band_[ispin][ik][iband].real() * Ht_in_eV
                     << std::endl;
            }
            *ost << std::endl;
        }
...
void BlochStates::bcast_eigenvalues(const std::string &calc_mode) {
    ...
    for (int ispin = 0; ispin < eigenvalues_ref_.size(); ispin++) {
        for (int ik = 0; ik < eigenvalues_ref_[ispin].size(); ik++) {
            MPI_Bcast(&eigenvalues_ref_[ispin][ik][0],
                      eigenvalues_ref_[ispin][ik].size(),

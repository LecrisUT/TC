...
void calc_hamiltonian::hf2x(
    ...
    double two_body_factor =
        -FourPI /
        (kpoints.num_kpoints() *
         crystal_structure.unit_cell_volume()); // "-": exchange, 4pi: Coulomb
            ...
            for (int jband = 0; jband < H2phi[ispin][ik].size(); jband++) {
                ...
                for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
                    for (int isymq = 0;
                        ...
                        for (int ibandq = -nbands_old;
                            ...
                            phiqj = phiq.conjugate().array() *
                                    phij.array(); // in R-space
                            plane_wave_basis.FFT_forward(phiqj, phiqj);
                            phiqj =
                                phiqj.array() * V_coulomb.array(); // in G-space
                ...
                if (potentials.includes_div_correction()) {
                    ...
                    for (int ibandk = -nbands_old; ibandk < num_bands_tc[ispin];
                        ...
                        Complex prod = phik_ref.dot(
                            phi[ispin][ik][jband][0]); // conj(phik)*phi

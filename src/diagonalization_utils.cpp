...
bool Diagonalization::Gram_Schmidt(std::vector<Eigen::VectorXcd> &vectors,
    ...
    for (int iband = 0; iband < target_band; iband++) {
        coeff(iband) =
            vectors[iband].dot(target_vector); // <iband | target_vector>
        // If you comment-out the below two lines, unmodified Gram Schmidt is
        // performed.
        //  }
        //  for (int iband=0; iband<target_band; iband++) {
        target_vector -=
            coeff(iband) * vectors[iband]; // |target_vector> -= <iband |
                                           // target_vector> * |iband>
    }
...
void Diagonalization::sort_eigen(Eigen::VectorXcd &eigenvalues,
...
void Diagonalization::make_a_new_trial_vector(const Eigen::VectorXd &kGvect2,
    ...
    //    const Complex expectation_value = phi.dot(H1phi + H2phi + H3phi); //
    //    <phi|H|phi>
    ...
    for (int ipw_at_k = 0; ipw_at_k < num_G_at_k; ipw_at_k++) {
        ...
        coeff_precond(ipw_at_k) =
            (27 + k_ratio * (18 + k_ratio * (12 + k_ratio * 8))) /
            (27 +
             k_ratio * (18 + k_ratio * (12 + k_ratio * (8 + k_ratio * 16))));
    }
    if (uses_3body) {
        phi = -coeff_precond.array() *
              (H1phi + H2phi + H3phi - eigenvalue * phi).array();
    } else {
        phi =
            -coeff_precond.array() * (H1phi + H2phi - eigenvalue * phi).array();
    }

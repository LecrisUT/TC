...
class CrystalStructure {
  private:
    ...
    // (in bohr^{-1}. e.g., lattice_vectors_ * reciprocal_vectors_.transpose() =
    // 2pi*identity
    ...
    std::vector<Eigen::Vector3d>
        atomic_position_cartesian_; // cartesian coordinate of each atom (in
                                    // bohr)

    // Note: num. of total atoms = index_of_atoms_.size() =
    // coord_cartesian_atoms_.size()

  public:
    const Eigen::Matrix3d &lattice_vectors() const { return lattice_vectors_; }
    const Eigen::Matrix3d &reciprocal_vectors() const {
        return reciprocal_vectors_;
    }
    ...
    const std::vector<Eigen::Vector3d> &atomic_position_cartesian() const {
        return atomic_position_cartesian_;
    }

    CrystalStructure()
        : unit_cell_volume_(0.0), num_atoms_(0), num_atomic_species_(0) {}

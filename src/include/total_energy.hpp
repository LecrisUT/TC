...
class TotalEnergy {
  private:
    ...
    std::vector<std::vector<std::vector<Complex>>>
        energy_1body_; // <phi|H_1body|phi> (for BITC, <phi_left|H_1body|phi>)
  ...
  public:
    const std::vector<std::vector<std::vector<Complex>>> &energy_1body() const {
        return energy_1body_;
    }
    const std::vector<std::vector<std::vector<Complex>>> &energy_2body() const {
        return energy_2body_;
    }
    const std::vector<std::vector<std::vector<Complex>>> &energy_3body() const {
        return energy_3body_;
    }
    ...
    TotalEnergy()
        : total_energy_(0.0), total_energy_difference_(0.0),
          total_energy_1body_(0.0), total_energy_2body_(0, 0),
          total_energy_3body_(0, 0), ewald_energy_(0.0) {}
    ...
    void bcast_qe_input(const int &num_independent_spins,
                        ...
                        const std::string &calc_method, const bool is_heg);

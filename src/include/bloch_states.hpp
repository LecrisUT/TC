...
class BlochStates {
  private:
    ...
    // num_bands is array: (spin-polarized && non-collinear) num_bands[0]=up,
    // num_bands[1]=down. (otherwise) num_bands[0].
    std::vector<int> num_bands_qe_; // num. of bands in Quantum Espresso
    // num. of bands in TC++ for the SCF or BAND orbitals, should satisfy
    // "num_bands_scf/band_ <= num_bands_qe_"
    std::vector<int> num_bands_scf_;
    std::vector<int> num_bands_band_;

    // num_occupied_bands_[spin][kpoint]
    std::vector<std::vector<int>>
        num_occupied_bands_; // num. of bands where filling_ > 1e-8

    // eigenvalues[spin][kpoint][band]. Num. of spin indicies = 2 for
    // spin-polarized && non-collinear, otherwise 1. (same as num_bands)
    std::vector<std::vector<std::vector<double>>> eigenvalues_qe_;
    std::vector<std::vector<std::vector<Complex>>>
        eigenvalues_scf_; // Note: TC Hamiltonian is non-Hermitian
    std::vector<std::vector<std::vector<Complex>>>
        eigenvalues_band_; // Note: TC Hamiltonian is non-Hermitian
    std::vector<std::vector<std::vector<double>>>
        filling_; // 0<=filling<=2 for spin-unpolarized && collinear. otherwise,
                  // 0<=filling<=1.

    double
        density_difference_; // L1 norm of the density difference between this
                             // and previous SCF loops (to check convergence)

    // phik[spin][k-point][band][num_spinor][npw]: wave function in G-space at
    // each irreducible k-point
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_scf_;
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_left_scf_; // for method.calc_method_=="BITC"
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_band_;
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_left_band_; // for method.calc_method_=="BITC"
    std::vector<Eigen::VectorXcd>
        density_; // 0: up, 1: down. in R-space. Note: complex for BITC method.

    // old wave functions for mixing (mixes_density_matrix==true)
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_scf_old_;
    std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        phik_left_scf_old_; // for method.calc_method_=="BITC"
  ...
  public:
    ...
    const std::vector<std::vector<int>> &num_occupied_bands() const {
        return num_occupied_bands_;
    }
    const std::vector<std::vector<std::vector<double>>> &
    eigenvalues_qe() const {
        return eigenvalues_qe_;
    }
    const std::vector<std::vector<std::vector<Complex>>> &
    eigenvalues_scf() const {
        return eigenvalues_scf_;
    }
    const std::vector<std::vector<std::vector<Complex>>> &
    eigenvalues_band() const {
        return eigenvalues_band_;
    }
    const std::vector<std::vector<std::vector<double>>> &filling() const {
        return filling_;
    }
    ...
    const std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>> &
    phik_left_band() const {
        return phik_left_band_;
    }
    const std::vector<Eigen::VectorXcd> &density() const { return density_; }
    const std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>> &
    phik_scf_old() const {
        return phik_scf_old_;
    }
    const std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>> &
    phik_left_scf_old() const {
        return phik_left_scf_old_;
    }
    const std::vector<std::vector<std::vector<double>>> &filling_old() const {
        return filling_old_;
    }

    BlochStates()
        : num_electrons_(-1.0), num_bands_qe_{-1}, num_bands_scf_{-1},
          num_bands_band_{-1}, density_difference_(0.0) {}

    // [bloch_states_initialize.cpp]
    // set_num_bands_tc() &
    // set_qe_xml() & resize_phik_qe() & set_phik_qe() & bcast_qe_input() &
    // bcast_scfinfo() & set_phik_scf_from_tc_wfc() &
    // set_eigenvalues_scf_from_tc_energy()
    ...
    void bcast_qe_input(const PlaneWaveBasis &plane_wave_basis,
                        const bool is_spinor, const std::string &calc_mode,
    ...
    void bcast_scfinfo(const PlaneWaveBasis &plane_wave_basis,
                       const bool is_spinor, const std::string &calc_method,
                       const bool am_i_mpi_rank0);
    ...
    void
    set_eigenvalues_from_tc_energy(const int &ispin, const int &ik,
    ...
    // called only when diagonalization.mixes_density_matrix==true && not the
    // first loop
    ...
    void set_density(const CrystalStructure &crystal_structure,
                     ...
                     const double &mixing_beta, const bool is_first_iter,
                     const bool is_bitc, const bool am_i_mpi_rank0,
                     std::ostream *ost);
    ...
    void reset_phik(const int &ispin, const int &ik,
                    ...
                    const bool mixes_density_matrix,
                    const std::string &calc_mode);
    void bcast_phik(const bool bcast_phik_left, const bool mixes_density_matrix,
                    const std::string &calc_mode, const bool am_i_mpi_rank0);

...
class Spin {
  private:
    // cf. in Quantum Espresso, nspin = 1 for no-spin, 2 for spin-polarized
    // collinear, 4 for non-collinear To summarize, (is_spinor_,
    // num_independent_spins_) = [no-spin: nspin = 1 in QE] (false, 1)
    // [spin-polarized collinear: nspin = 2 in QE] (false, 2)
    // [non-collinear: nspin = 4 in QE] (true, 1)
    bool is_spinor_;
    int num_independent_spins_; // (no-spin or non-collinear) 1, (spin-polarized
                                // spin-collinear) 2

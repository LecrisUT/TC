...
class FileNames {
  private:
    // "reads_binary_" is applied to qe_wfc. When reads_binary_==false, read
    // non-binary files. (default: true) This variables is prepared only for
    // test calculation where environment-dependent binary files are not
    // appropriate...
    bool reads_binary_;
    ...
    const std::string tc_wfc_scf_;  // TC++ output (SCF wave functions) binary
                                    // file name (tc_wfc_scf.dat)
    const std::string tc_wfc_band_; // TC++ output (BAND wave functions) binary
                                    // file name (tc_wfc_band.dat)
    const std::string tc_energy_scf_;  // TC++ output (SCF orbital energies)
                                       // binary file name (tc_energy_scf.dat)
    const std::string tc_energy_band_; // TC++ output (BAND orbital energies)
                                       // binary file name (tc_energy_band.dat)
    const std::string tc_scfinfo_; // TC++ output. SCF information that will be
    ...
    // Address of the QE (quantum-espresso) "save" directory. e.g.
    // "/home/hoge/QE/Si/prefix.save" xml and wfc files are placed in the QE
    // "save" directory specified here.
    std::string qe_save_dir_;

    std::string
        qe_xml_; //  e.g. "/home/hoge/QE/Si/prefix.save/data-file-schema.xml"

    // (spin-polarized, collinaer) qe_wfc_[is][ik]  is = 0 (up) or 1 (down), ik
    // = irreducible k-point e.g. "/home/hoge/QE/Si/prefix.save/wfcup3.dat"
    // (is=0, ik=3) ".../wfcdw4.dat" (is=1, ik=4) (no-spin or non-collinear)
    // qe_wfc_[is][ik]  is = 0, ik = irreducible k-point e.g.
    // "/home/hoge/QE/Si/prefix.save/wfc3.dat" (is=0, ik=3)
    ...
    std::vector<std::string>
        upf_; // e.g. pseudo_dir_ + "Si.upf". upf_[index of atomic species]
  ...
  public:
    ...
    const std::vector<std::vector<std::string>> &qe_wfc() const {
        return qe_wfc_;
    }
    ...
    FileNames()
        : reads_binary_(true), tc_input_("input.in"), tc_output_("output.out"),
          tc_wfc_scf_("tc_wfc_scf.dat"), tc_wfc_band_("tc_wfc_band.dat"),
          tc_energy_scf_("tc_energy_scf.dat"),
          tc_energy_band_("tc_energy_band.dat"), tc_scfinfo_("tc_scfinfo.dat"),
          tc_bandplot_("tc_bandplot.dat"),
          tc_bandplot_up_("tc_bandplot_up.dat"),
          tc_bandplot_dn_("tc_bandplot_dn.dat"), qe_save_dir_(""),
          pseudo_dir_("") {}

    void set_qe_save_dir(const std::string &qe_save_dir);
    void set_qe_xml_file_name(); // NOTE! should be called after set_qe_save_dir
    void set_qe_wfc_file_names(
        const Spin &spin,
        const int &num_irreducible_kpoints); // NOTE! should be called after
                                             // set_qe_save_dir
    void set_pseudo_dir(const std::string &pseudo_dir);
    void
    set_upf_file_names(const std::vector<std::string>
                           &upf); // NOTE! should be called after set_pseudo_dir

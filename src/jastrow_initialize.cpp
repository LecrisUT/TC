...
// An appropriate factor is later multiplied in unnormalize_A_long() to make it
// in Hartree unit. F_long_ is also initialized there.
void Jastrow::set_A_long_normalized(
    const std::vector<std::vector<double>> &A_long) {
    assert(A_long_.size() == 2 && A_long_[0].size() == 2 &&
           A_long_[1].size() == 2);
    assert(std::abs(A_long[0][1] - A_long[1][0]) <
           1e-8); // A_long_up_dn = A_long_dn_up
    A_long_ = A_long;
}
...
// In "input.in", multiply "sqrt(V/(4piN))" (V:volume, N:no. of valence
// electrons) with the A parameter to make it in Hartree unit. F_long is also
// determined so as to satisfy the cusp condition.
void Jastrow::unnormalize_A_long(const CrystalStructure &crystal_structure,
                                 const BlochStates &bloch_states,
                                 const Spin &spin, std::ostream *ost) {
    assert(A_long_.size() == 2 && A_long_[0].size() == 2 &&
           A_long_[1].size() == 2);
    assert(F_long_.size() == 2 && F_long_[0].size() == 2 &&
           F_long_[1].size() == 2);
    if (spin.num_independent_spins() == 1 &&
        !spin.is_spinor()) // no-spin, up & down are equivalent
    {
        if (std::abs(A_long_[0][0] - A_long_[1][1]) > 1e-8) {
            error_messages::stop(
                "This calculation assumes that up & down spins are equivalent, "
                "so please specify the same value for A_up_up & A_dn_dn");
        }
    }
    ...
    const double num_electrons =
        bloch_states.num_electrons(); // including the spin degree of freedom
    ...
    for (int is1 = 0; is1 < 2; is1++) {
        for (int is2 = 0; is2 < 2; is2++) {
            ...
            if (std::abs(A_long_[is1][is2]) > 1e-8) {
                if (is1 == is2) {
                    F_long_[is1][is2] = std::sqrt(2 * A_long_[is1][is2]);
                }
                if (is1 != is2) {
                    F_long_[is1][is2] = std::sqrt(A_long_[is1][is2]);
                }
            }
        } // is2
    }     // is1
...
void Jastrow::bcast() {
    ...
    assert(F2_long_.size() == 2 && F2_long_[0].size() == 2 &&
           F2_long_[1].size() == 2);
    assert(Finv2_long_.size() == 2 && Finv2_long_[0].size() == 2 &&
           Finv2_long_[1].size() == 2);
    assert(PI_AAFinv2_long_.size() == 2 && PI_AAFinv2_long_[0].size() == 2 &&
           PI_AAFinv2_long_[1].size() == 2);
    for (int is1 = 0; is1 < 2; is1++) {
        for (int is2 = 0; is2 < 2; is2++) {
            assert(std::abs(F_long_[is1][is2]) >
                   1e-8); // guaranteed in normalize_A_long()
            ...
            PI_AAFinv2_long_[is1][is2] = PI * A_long_[is1][is2] *
                                         A_long_[is1][is2] *
                                         Finv2_long_[is1][is2];
        }
    }

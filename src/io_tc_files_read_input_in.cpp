...
bool read_bool(const std::string &keyword, const std::string &s2) {
    if (s2 == "true" || s2 == "True" || s2 == "TRUE" || s2 == "T" ||
        s2 == "t" || s2 == ".true." || s2 == ".True." || s2 == ".TRUE.") {
        return true;
    } else if (s2 == "false" || s2 == "False" || s2 == "FALSE" || s2 == "F" ||
               s2 == "f" || s2 == ".false." || s2 == ".False." ||
               s2 == ".FALSE.") {
        return false;
    } else {
        error_messages::inappropriate_argument(keyword, s2,
...
void already_set_error(const std::string &s1, std::ostream *ost) {
    *ost << "The keyword " << s1 << " is set twice in input.in!" << std::endl;
    error_messages::stop("Error in reading input.in (a keyword is specified "
                         "twice). See output.out for details.");
}
...
void io_tc_files::read_input_in(FileNames &file_names,
    ...
    if (am_i_mpi_rank0) // open input.in
    {
        *ost << " Read TC input (" << file_names.tc_input() << ")" << std::endl;
        ...
        if (ifs.fail()) {
            error_messages::cannot_open(file_names.tc_input());
        }
        ...
        std::vector<bool> is_already_set(
            21, false); // 21 = total num. of the input keywords
        while (std::getline(ifs, sline)) {
            std::stringstream ss{sline};
            s1 = s2 = "";   // initialization is necessary considering the case
                            // when sline=="" (a void line)
            ss >> s1 >> s2; // Note that, when sline=="", this operation is not
                            // performed.
            if (s1 == "calc_method") {
                if (is_already_set[0]) {
                    already_set_error(s1, ost);
                }
                ...
                if (is_already_set[18]) {
                    already_set_error(s1, ost);
                }
                A_long[1][1] = boost::lexical_cast<double>(s2);
                is_already_set[18] = true;
            } else if (s1 == "mixes_density_matrix") {
                if (is_already_set[19]) {
                    already_set_error(s1, ost);
                }
                mixes_density_matrix = read_bool(s1, s2);
                is_already_set[19] = true;
            } else if (s1 == "reads_binary") {
                if (is_already_set[20]) {
                    already_set_error(s1, ost);
                }
                reads_binary = read_bool(s1, s2);
                is_already_set[20] = true;
            } else if (s1 != "" && s1.c_str()[0] != '#' &&
                       s1 != "num_bands_tc") // neither a void line nor a
                                             // comment line
            {
                *ost << "Error in reading input.in. Your keyword " << s1
                     << " is not valid. " << std::endl;
                *ost << "(If you would like write to a comment, please note "
                        "that a comment line should begin with #)"
                     << std::endl;
                error_messages::stop("Error in reading input.in (invalid "
                                     "keyword). See output.out for details.");
            }
        }
        if (!is_already_set[0]) {
            error_messages::stop(
                "The keyword calc_method should be specified in input.in");
        }
        if (!is_already_set[1]) {
            error_messages::stop(
                "The keyword calc_mode should be specified in input.in");
        }
        if (!is_already_set[2]) {
            error_messages::stop(
                "The keyword qe_save_dir should be specified in input.in");
        }
        if (!is_already_set[3]) {
            error_messages::stop(
                "The keyword pseudo_dir should be specified in input.in");
        }
        if (!((is_already_set[16] && is_already_set[17] &&
               is_already_set[18]) ||
              (!is_already_set[16] && !is_already_set[17] &&
               !is_already_set[18]))) {
            error_messages::stop(
                "Jastrow A parameters (A_up_up, A_up_dn, A_dn_dn) are only "
                "partially given in input.in");
        }

        ifs.clear();
        ifs.seekg(0, std::ios_base::beg);
        while (std::getline(ifs, sline)) // read num_bands_tc (can be
                                         // num_bands_scf/band, need calc_mode!)
        {
            std::stringstream ss{sline};
            s1 = s2 = "";   // initialization is necessary considering the case
                            // when sline=="" (a void line)
            ...
            if (s1 == "num_bands_tc") {
                ...
                bloch_states.set_num_bands_tc({boost::lexical_cast<int>(s2)},
                                              method.calc_mode());
        ...
        diagonalization.set(restarts, energy_tolerance, charge_tolerance,
                            max_num_iterations, mixes_density_matrix,
                            mixing_beta, num_refresh_david,
                            max_num_blocks_david, method.calc_mode(), is_heg);
        potentials.set_includes_div_correction(includes_div_correction);
        potentials.set_is_heg(is_heg);
        potentials.jastrow.set(A_long);
        if (!reads_binary) {
            file_names.set_reads_binary_false(ost);
        }

        // Show parameters set here (if not specified in the input file, a
        ...
        *ost << "  pseudo_dir = " << file_names.pseudo_dir() << std::endl;
        ...
        const std::vector<int> &num_bands_tc =
            method.calc_mode() == "SCF" ? bloch_states.num_bands_scf()
                                        : bloch_states.num_bands_band();
        if (num_bands_tc[0] >= 0) {
            *ost << "  num_bands_tc = " << num_bands_tc[0] << std::endl;
        }

        *ost << "  smearing_mode = " << kpoints.smearing_mode() << std::endl;
        *ost << "  smearing_width = " << kpoints.smearing_width() << std::endl;

        *ost << "  restarts = " << bool_to_string(diagonalization.restarts())
             << std::endl;
        *ost << "  includes_div_correction = "
             << bool_to_string(potentials.includes_div_correction())
             << std::endl;
        *ost << "  energy_tolerance = " << diagonalization.energy_tolerance()
             << std::endl;
        *ost << "  charge_tolerance = " << diagonalization.charge_tolerance()
             << std::endl;
        *ost << "  max_num_iterations = "
             << diagonalization.max_num_iterations() << std::endl;
        *ost << "  mixes_density_matrix = "
             << bool_to_string(diagonalization.mixes_density_matrix())
             << std::endl;
        *ost << "  mixing_beta = " << diagonalization.mixing_beta()
             << std::endl;
        *ost << "  num_refresh_david = " << diagonalization.num_refresh_david()
             << std::endl;
        *ost << "  max_num_blocks_david = "
             << diagonalization.max_num_blocks_david() << std::endl;
        *ost << "  is_heg = " << bool_to_string(potentials.is_heg())
             << std::endl;
        if (method.calc_method() == "TC" || method.calc_method() == "BITC") {
            *ost << "  A_up_up (unit: sqrt(V/(4piN)) ) = "
                 << potentials.jastrow.A_long()[0][0] << std::endl;
            *ost << "  A_up_dn (unit: sqrt(V/(4piN)) ) = "
                 << potentials.jastrow.A_long()[0][1] << std::endl;
            *ost << "  A_dn_up (unit: sqrt(V/(4piN)) ) = "
                 << potentials.jastrow.A_long()[1][0] << std::endl;
            *ost << "  A_dn_dn (unit: sqrt(V/(4piN)) ) = "
                 << potentials.jastrow.A_long()[1][1] << std::endl;
        }
        if (!reads_binary) {
            *ost << "  reads_binary = " << file_names.reads_binary()
                 << std::endl;
        }
    ...
    bcast_input_in(file_names, diagonalization, method, potentials, kpoints,
...
void io_tc_files::bcast_input_in(FileNames &file_names,
                                 ...
                                 const bool am_i_mpi_rank0, std::ostream *ost) {
    if (am_i_mpi_rank0) {
        *ost << " Bcast TC input (" << file_names.tc_input() << ")"
             << std::endl;
    }
    ...
    // because this variable is possibly initialized by num_band_qe_ in
    // read_qe_xml().
    potentials.bcast_tc_input();
    // Jatrow parameters will be bcast later (after normalization using QE
    // variables)

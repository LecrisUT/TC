...
void get_array(std::ifstream &ifs, std::vector<double> &rvec,
    ...
    int imesh = 0;
    while (std::getline(ifs, sline)) {
        if (sline.find("/" + tag_name) != std::string::npos) {
            break;
        } // end reading
...
std::string read_upf_element(const boost::property_tree::ptree &pt,
...
void read_upf::read_upf(FileNames &file_names,
    ...
    if (am_i_mpi_rank0) // open upf files
    {
        *ost << " Read_upf" << std::endl;
        ...
        // std::vector<int> lmax(num_atomic_species); // maximum angular
        // momentum

        std::vector<std::vector<double>> pseudo_rmesh(
            num_atomic_species); // radial mesh
        // \int f(r) dr = \sum_i c_i f(i)*weight_rmesh(i). c_i = coefficients in
        // Simpson's rule
        std::vector<std::vector<double>> pseudo_weight_rmesh(
            num_atomic_species);
        std::vector<std::vector<double>> pseudo_local(
            num_atomic_species); // local potential
        // std::vector<std::vector<double> > pseudo_nlcc(num_atomic_species); //
        // non-linera core correction

        // Kleinman-Bylander projectors (pseudo_beta), their angular momenta
        // (pseudo_lbeta), and their coefficients (pseudo_dij)
        std::vector<std::vector<std::vector<double>>> pseudo_beta(
            num_atomic_species);
        std::vector<std::vector<int>> pseudo_lbeta(num_atomic_species);
        std::vector<Eigen::MatrixXd> pseudo_dij(num_atomic_species);

        for (int iatomic_species = 0; iatomic_species < num_atomic_species;
             iatomic_species++) {
            *ost << "  Species = " << iatomic_species << std::endl;
            std::ifstream ifs(file_names.upf()[iatomic_species]);
            if (ifs.fail()) {
                error_messages::cannot_open(file_names.upf()[iatomic_species]);
            }

            // read upf
            bool is_upf_ver2 = check_upf_ver2(
                ifs, ost);      // UPF ver.2 or ver.1 (XML schema not supported)
            int mesh_size;      // num. of radial mesh grid
            int num_projectors; // num. of Kleinman-Bylander projectors
            read_upf_header(ifs, file_names.upf()[iatomic_species], is_upf_ver2,
                            Z_valence[iatomic_species], mesh_size,
                            num_projectors, ost);
            ...
            for (int iproj = 0; iproj < num_projectors; iproj++) {
                pseudo_beta[iatomic_species][iproj].resize(mesh_size);
            }
            ...
            // read upf
            read_upf_mesh(ifs, pseudo_rmesh[iatomic_species],
                          pseudo_weight_rmesh[iatomic_species]);
            read_upf_local(ifs, pseudo_local[iatomic_species]);
            read_upf_nonlocal(ifs, is_upf_ver2, pseudo_beta[iatomic_species],
                              pseudo_lbeta[iatomic_species],
                              pseudo_dij[iatomic_species], ost);
            // if (is_nlcc_included[iatomic_species]) { read_upf_nlcc(ifs,
            // pseudo_nlcc[iatomic_species]); }
        }
        if (am_i_mpi_rank0) {
...
// read PP_HEADER
void read_upf::read_upf_header(std::ifstream &ifs,
                               const std::string &file_name_upf,
                               const bool is_upf_ver2, double &Z_valence,
                               int &mesh_size, int &num_projectors,
                               std::ostream *ost) {
    std::string sline, s1;
    std::stringstream ss;

    if (!find_tag(ifs, "PP_HEADER")) {
        error_messages::stop("Error: cannot find PP_HEADER");
    }
    ...
    if (is_upf_ver2) // UPF ver.2
    {
        ...
        std::string pseudo_type =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.pseudo_type", ost);
        std::string is_ultrasoft =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.is_ultrasoft", ost);
        std::string is_paw =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.is_paw", ost);
        std::string is_coulomb =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.is_coulomb", ost);

        // pseudo type (should be norm-conserving pseudopot.)
        if (pseudo_type == "US" || pseudo_type == "PAW" ||
            is_ultrasoft == "T" || is_paw == "T" || is_coulomb == "T") {
            error_messages::stop(
                "Ultrasoft, PAW, 1/r (is_coulomb=\"T\") are not supported.");
        }

        std::string core_correction = read_upf_element(
            pt, "UPF.PP_HEADER.<xmlattr>.core_correction", ost);
        // nonlinear core correction
        if (core_correction == "T") {
            error_messages::stop(
                "Non-linear core correction is not supported...");
        } else if (core_correction == "F") {
            *ost
                << "   Non-linear core correction is not included in pseudopot."
                << std::endl;
        }

        std::string Z_valence_string =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.z_valence", ost);
        Z_valence = std::stod(Z_valence_string);
        *ost << "   Z_valence = " << Z_valence << std::endl;

        std::string mesh_size_string =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.mesh_size", ost);
        mesh_size = std::stoi(mesh_size_string);
        *ost << "   mesh_size = " << mesh_size << std::endl;

        std::string number_of_proj_string =
            read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.number_of_proj", ost);
    ...
    } else // UPF ver.1
    {
        ...
        if (s1 != "NC") {
            error_messages::stop("Ultrasoft, PAW, 1/r are not supported.");
        }

        s1 = getline_and_return_first(
            ifs); // 4th line: nonlinear core correction
        if (s1 == "T") {
            error_messages::stop(
                "Non-linear core correction is not supported...");
        } else {
        ...
        Z_valence =
            std::stod(getline_and_return_first(ifs)); // 6th line: Z_valence
        *ost << "   Z_valence = " << Z_valence << std::endl;
        ...
        // lmax = std::stoi(getline_and_return_first(ifs)); // 9th line: maximum
        // angular momentum (lmax) *ost << "   lmax: " << lmax << std::endl;

        mesh_size = std::stoi(getline_and_return_first(
            ifs)); // 10th line: number of points in the radial grid
        *ost << "   mesh_size = " << mesh_size << std::endl;

        num_projectors = std::stoi(getline_and_return_second(
            ifs)); // 11th line: (second argument) num. of projectors
        *ost << "   num. of projectors = " << num_projectors << std::endl;
    }
}

// read PP_MESH
void read_upf::read_upf_mesh(std::ifstream &ifs,
                             std::vector<double> &pseudo_rmesh,
...
// read PP_LOCAL
void read_upf::read_upf_local(std::ifstream &ifs,
                              std::vector<double> &pseudo_local) {
    ifs.clear();
    ifs.seekg(0, std::ios_base::beg);
    ...
    if (is_upf_ver2) // UPF ver.2
    {
        ...
        for (int iproj = 0; iproj < num_projectors; iproj++) {
            while (std::getline(ifs, sline)) {
                ...
                {
                    while (sline.find("angular_momentum") ==
                    ...
                    *ost << "    angular momentum = " << pseudo_lbeta[iproj]
                         << std::endl;

                    while (sline.find(">") == std::string::npos) // not found
                    {
                        std::getline(ifs,
                                     sline); // getline() until reaching ">"
                    }
                    is_beta_found = true;
                    break;
                }
            ...
            if (!is_beta_found) {
                error_messages::stop("Error: PP_BETA not found.");
            }

            int imesh = 0;
            while (std::getline(ifs, sline)) {
                if (sline.find("/PP_BETA") != std::string::npos) {
                    break;
                } // end reading
        ...
        for (int iproj = 0; iproj < num_projectors; iproj++) {
            for (int jproj = 0; jproj < num_projectors; jproj++) {
                pseudo_dij(iproj, jproj) =
                    tmp_dij[iproj * num_projectors + jproj];
            }
        }
    } else // UPF ver.1
    {
        for (int iproj = 0; iproj < num_projectors; iproj++) {
            if (!find_tag(ifs, "PP_BETA")) {
                error_messages::stop("Error: PP_BETA not found.");
            }

            pseudo_lbeta[iproj] =
                std::stoi(getline_and_return_second(ifs)); // angular momentum
            *ost << "    angular momentum = " << pseudo_lbeta[iproj]
                 << std::endl;
        ...
        if (!find_tag(ifs, "PP_DIJ")) {
            error_messages::stop("Error: PP_DIJ not found");
        }
        int num_elements = std::stoi(
            getline_and_return_first(ifs)); // num. of non-zero dij elements
        pseudo_dij = Eigen::MatrixXd::Zero(num_projectors, num_projectors);

        int icheck = 0;
        while (std::getline(ifs, sline)) {
            if (sline.find("/PP_DIJ") != std::string::npos) {
                break;
            } // end reading
...
void read_upf::read_upf_nlcc(std::ifstream &ifs,
                             std::vector<double> &pseudo_nlcc) {
    ifs.clear();
    ifs.seekg(0, std::ios_base::beg);

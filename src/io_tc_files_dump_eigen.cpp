...
void io_tc_files::dump_eigen(const FileNames &file_names, const Method &method,
...
void io_tc_files::dump_wfc(const FileNames &file_names, const Method &method,
                           const std::string &calc_mode,
    ...
    const std::vector<std::vector<std::vector<std::vector<Eigen::VectorXcd>>>>
        &phik_left_ref = calc_mode == "SCF" ? bloch_states.phik_left_scf()
                                            : bloch_states.phik_left_band();

    if (calc_mode == "SCF") {
        *ost << " Dump SCF wave functions (" << file_name_ref << ")"
             << std::endl;
    } else if (calc_mode == "BAND") {
        *ost << " Dump BAND wave functions (" << file_name_ref << ")"
             << std::endl;
    }
    std::ofstream ofs(file_name_ref,
    ...
    ofs.write(reinterpret_cast<const char *>(&num_independent_spins),
              sizeof(int));
    const int num_irreducible_kpoints_scf = phik_ref[0].size();
    ofs.write(reinterpret_cast<const char *>(&num_irreducible_kpoints_scf),
              sizeof(int));
            ...
            for (int iband = 0; iband < phik_ref[ispin][ik].size(); iband++) {
                for (int ispinor = 0;
                    ...
                    for (int ipw_at_k = 0;
                        ...
                        // not a clever way...
                        double re =
                            phik_ref[ispin][ik][iband][ispinor](ipw_at_k)
                                .real();
                        double im =
                            phik_ref[ispin][ik][iband][ispinor](ipw_at_k)
                                .imag();
                        ofs.write(reinterpret_cast<const char *>(&re),
                                  sizeof(double));
                        ofs.write(reinterpret_cast<const char *>(&im),
                                  sizeof(double));
                    } // ipw_at_k
                }     // ispinor
            }         // iband
                ...
                for (int iband = 0; iband < phik_ref[ispin][ik].size();
                    ...
                    for (int ispinor = 0;
                        ...
                        for (int ipw_at_k = 0;
                             ...
                             ipw_at_k++) {
                            double re =
                                phik_left_ref[ispin][ik][iband][ispinor](
                                    ipw_at_k)
                                    .real();
                            double im =
                                phik_left_ref[ispin][ik][iband][ispinor](
...
void io_tc_files::dump_energy(const FileNames &file_names, const Method &method,
                              const std::string &calc_mode,
                              const BlochStates &bloch_states,
                              std::ostream *ost) {
    assert(calc_mode == "SCF" || calc_mode == "BAND");
    ...
    if (calc_mode == "SCF") {
        *ost << " Dump SCF orbital energies (" << file_name_ref << ")"
             << std::endl;
    } else if (calc_mode == "BAND") {
        *ost << " Dump BAND orbital energies (" << file_name_ref << ")"
             << std::endl;
    }
    std::ofstream ofs(file_name_ref,
                      std::ios::out | std::ios::binary); // binary

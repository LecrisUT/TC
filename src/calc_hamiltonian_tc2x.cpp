...
void calc_hamiltonian::tc2x(
    ...
    const int num_irreducible_kpoints_ref =
        method.calc_mode() == "SCF" ? // kpoints for "phi"
            kpoints.num_irreducible_kpoints_scf()
    ...
    double two_body_factor =
        -1.0 / (kpoints.num_kpoints() *
                crystal_structure.unit_cell_volume()); // "-": exchange
    if (num_independent_spins == 1) {
        two_body_factor /= 2.0;
    } // exchange: parallel spins only, "filling" should be divided by 2
    // coefficient of [2b_x1]: FFT I^2 = -1 is additionally multiplied
    // coefficient of [2b_x2]: FFT I^2 * convolution (\nabla_2 u_21 = "-1" *
    // \nabla_1 u_12) = +1.

    std::vector<Eigen::Vector3d> Gvect(
        plane_wave_basis
            .size_FFT_grid()); // Gvect[ipw](idim) for 3x3 matrix operation
    ...
    for (int idim = 0; idim < 3; idim++) {
        qGvect[idim].resize(
            plane_wave_basis.size_FFT_grid()); // kqGvect[idim](ipw)
    }
        ...
        for (int ik = 0; ik < num_irreducible_kpoints_ref; ik++) {
            ...
            if (potentials.includes_div_correction()) {
                kVaux = method.calc_mode() == "BAND"
                            ...
                            : potentials.sum_of_kVaux_scf()[ik] /
                                  (kpoints.num_kpoints() *
                                   crystal_structure.unit_cell_volume());
                // (-1) of exchange interaction is cancelled with the other
                // "-1":
                //  \int \nabla Vaux - \sum \nabla Vaux = 0 - sum_of_kVaux =
                //  "-1" * sum_of_kVaux.
                if (num_independent_spins == 1) {
                    kVaux /= 2.0;
                } // same as two_body_factor
                kVaux *=
                    potentials.jastrow
                ...
                if (kVaux.squaredNorm() >
                    1e-8) // if = 0 then div_corr_2bx1/x2 is not needed
                {
                    ...
                    for (int ibandk = -nbands_old; ibandk < num_bands_tc[ispin];
                         ibandk++) {
                        double filling_ibandk =
                            ibandk >= 0
                                ...
                                : bloch_states
                                      .filling_old()[ispin][ik][-1 - ibandk];

                        if (ibandk >= 0 && method.calc_mode() == "SCF") {
                            // above filling_ibandk
                            // (=filling[ispin][ik][ibandk]) is not correct for
                            // zero-weight k-points in SCF calc.
                            if (kpoints.kweight_scf()[ik] < 1e-8) {
                                filling_ibandk = kpoints.return_band_filling(
                                    spin,
                                    bloch_states
                                        .eigenvalues_scf()[ispin][ik][ibandk]
                                        .real(),
                                    bloch_states.fermi_energy(),
                        ...
                        Eigen::VectorXcd &div_corr_2bx1_ref =
                            ibandk >= 0 ? div_corr_2bx1[ibandk]
                                        : div_corr_2bx1_old[-1 - ibandk];
                        std::vector<Eigen::VectorXcd> &div_corr_2bx2_ref =
                            ibandk >= 0 ? div_corr_2bx2[ibandk]
                                        : div_corr_2bx2_old[-1 - ibandk];

                        // [2b_x1]
                        div_corr_2bx1_ref = Eigen::VectorXcd::Zero(
                        ...
                        const Eigen::VectorXcd &phik_ref =
                            ibandk >= 0
                                ? (method.calc_mode() == "BAND"
                                       ? bloch_states
                                             .phik_band()[ispin][ik][ibandk][0]
                                       : bloch_states
                                             .phik_scf()[ispin][ik][ibandk][0])
                                : bloch_states.phik_scf_old()[ispin][ik]
                                                             [-1 - ibandk][0];

                        // Note: names "phiq" and "grad_phiq" are meaningless
                        // here.
                        ...
                        for (int idim = 0; idim < 3; idim++) {
                            div_corr_2bx1_ref +=
                                coeff_vec_2bx1(idim) *
                                grad_phiq[idim]; // NOTE! function in k-space.
                        }

                        // [2b_x2]
                        Eigen::VectorXcd &chi_ref_div =
                        ...
                        if (is_bitc) {
                            const Eigen::VectorXcd &chik_ref =
                                ibandk >= 0
                                    ? (method.calc_mode() == "BAND"
                                           ? bloch_states
                                                 .phik_left_band()[ispin][ik]
                                                                  [ibandk][0]
                                           : bloch_states
                                                 .phik_left_scf()[ispin][ik]
                                                                 [ibandk][0])
                                    : bloch_states
                                          .phik_left_scf_old()[ispin][ik]
                                                              [-1 - ibandk][0];
                        ...
                        for (int idim = 0; idim < 3; idim++) {
                            div_corr_2bx2_ref[idim] =
                                kVaux(idim) *
                                chi_ref_div; // NOTE! function in k-space.
                                             // conjugate() will be later taken.
                        }
                    } // ibandk
                }     // if (kVaux != 0 )
            ...
            for (int jband = 0; jband < H2phi[ispin][ik].size(); jband++) {
                ...
                plane_wave_basis.get_orbital_FFTgrid(
                    ispin, ik, 0, // isym = 0
                    false,        // time-rersal not used for isym=0
                    phi[ispin][ik][jband][0], phij, method.calc_mode());
                for (int ipw = 0; ipw < plane_wave_basis.size_FFT_grid();
                     ipw++) {
                    for (int idim = 0; idim < 3; idim++) {
                        grad_phij[idim](ipw) =
                            (kvect(idim) + Gvect[ipw](idim)) * phij(ipw);
                    }
                }
                for (int idim = 0; idim < 3; idim++) {
                    plane_wave_basis.FFT_backward(grad_phij[idim],
                                                  grad_phij[idim]);
                }
                ...
                for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
                    for (int isymq = 0;
                         isymq < kpoints.kvectors_scf()[iq].size(); isymq++) {
                        if (bloch_states.num_occupied_bands()[ispin][iq] == 0) {
                            continue;
                        }
                        ...
                        for (int ipw = 0;
                            ...
                            for (int idim = 0; idim < 3; idim++) {
                                qGvect[idim](ipw) =
                                    qvect(idim) +
                                    Gvect[ipw](
                                        idim); // NOTE: order of (ipw,idim)
                                kqGvect[idim](ipw) =
                                    kqvect(idim) + Gvect[ipw](idim);
                            }
                        }
                        ...
                        for (int ipw = 0;
                             ipw < plane_wave_basis.size_FFT_grid(); ipw++) {
                            V_2body(ipw) = potentials.jastrow.tc_2body(
                        ...
                        for (int ibandq = -nbands_old;
                            ...
                            if (ibandq >= 0) {
                                plane_wave_basis.get_orbital_FFTgrid(
                                    ispin, iq, isymq,
                                    kpoints.is_time_reversal_used_at_k()[iq]
                                                                        [isymq],
                                    bloch_states
                                ...
                                plane_wave_basis.get_orbital_FFTgrid(
                                    ...
                                    kpoints.is_time_reversal_used_at_k()[iq]
                                                                        [isymq],
                                    bloch_states.phik_scf_old()[ispin][iq]
                                                               [-1 - ibandq][0],
                                    phiq, "SCF");
                            }
                                ...
                                if (ibandq >= 0) {
                                    plane_wave_basis.get_orbital_FFTgrid(
                                        ...
                                        kpoints.is_time_reversal_used_at_k()
                                            [iq][isymq],
                                        bloch_states.phik_left_scf()[ispin][iq]
                                                                    [ibandq][0],
                                        chiq, "SCF");
                                } else {
                                    plane_wave_basis.get_orbital_FFTgrid(
                            ...
                            for (int idim = 0; idim < 3; idim++) {
                                grad_phiq[idim] =
                                    qGvect[idim].array() * phiq.array();
                                plane_wave_basis.FFT_backward(grad_phiq[idim],
                                                              grad_phiq[idim]);
                            }
                            plane_wave_basis.FFT_backward(phiq, phiq);

                            // [2a_x & 2b_x1]
                            phiqj = chiq_ref.conjugate().array() *
                                    phij.array(); // in R-space
                            plane_wave_basis.FFT_forward(phiqj,
                                                         phiqj); // -> phiqj(G)
                            ...
                            for (int idim = 0; idim < 3; idim++) {
                                phi_grad_phi[idim] =
                                    phiqj.array() *
                                    duk[idim].array(); // note: meaningless name
                                                       // "phi_grad_phi"
                                plane_wave_basis.FFT_backward(
                                    phi_grad_phi[idim], phi_grad_phi[idim]);
                            }

                            double filq =
                                ibandq >= 0
                                    ? bloch_states.filling()[ispin][iq][ibandq]
                                    : bloch_states.filling_old()[ispin][iq]
                                                                [-1 - ibandq];
                            // [2b_x1] <*,q| \nabla_1 u_12 \nabla_1 |q,j>
                            temp = Eigen::VectorXcd::Zero(
                                plane_wave_basis.size_FFT_grid());
                            for (int idim = 0; idim < 3; idim++) {
                                temp =
                                    temp.array() + phi_grad_phi[idim].array() *
                                                       grad_phiq[idim].array();
                            }
                            ...
                            // [2a_x] <*,q| V_2body |j,*>
                            phiqj =
                                phiqj.array() * V_2body.array(); // in G-space
                            // [2b_x2] <*,q| \nablba_2 u_12 \nabla_2 |*,j>
                            for (int idim = 0; idim < 3; idim++) {
                                phiqj = phiqj.array() +
                                        phi_grad_phi[idim].array() *
                                            duk[idim].array(); // in G-space
                            }
                            plane_wave_basis.FFT_backward(phiqj,
                                                          phiqj); // -> (R)
                            Hphij_sub = Hphij_sub.array() +
                                        filq * phiqj.array() *
                                            phiq.array(); // in R-space
                        }                                 // ibandq
                    }                                     // isymq
                }                                         // iq
                ...
                for (int ipw_at_k = 0; ipw_at_k < num_G_at_k; ipw_at_k++) {
                    H2phi[ispin][ik][jband][0](ipw_at_k) +=
                        two_body_factor * Hphij_sub(Gindex_at_k(ipw_at_k));
                }
                ...
                if (potentials.includes_div_correction()) {
                    ...
                    if (method.calc_mode() == "SCF") {
                        div_corr -= potentials.sum_of_Vaux_scf()[ik] /
                                    (kpoints.num_kpoints() *
                                     crystal_structure.unit_cell_volume());
                    } else if (method.calc_mode() == "BAND") {
                        div_corr -= potentials.sum_of_Vaux_band()[ik] /
                                    (kpoints.num_kpoints() *
                                     crystal_structure.unit_cell_volume());
                    }
                    ...
                    for (int ibandk = -nbands_old; ibandk < num_bands_tc[ispin];
                        ...
                        Complex prod = chik_ref.dot(
                            phi[ispin][ik][jband][0]); // conj(phii)*phij
                        prod *= div_corr;              // [2a_x]

                        if (kVaux.squaredNorm() >
                            1e-8) // \sum \nabla u correction (e.g. not required
                                  // for non-zero k-weight in SCF)
                        {
                            for (int ipw_at_k = 0; ipw_at_k < num_G_at_k;
                                 ipw_at_k++) {
                                // NOTE! function in k-space
                                H2phi[ispin][ik][jband][0](ipw_at_k) -=
                                    div_corr_2bx1_ref(Gindex_at_k(
                                        ipw_at_k)); // [2b_x1] I^2 = -1
                            }
                            for (int idim = 0; idim < 3; idim++) {
                                // NOTE! inner product in k-space
                                plane_wave_basis.FFT_forward(
                                    grad_phij[idim],
                                    grad_phiq[idim]); // save in "grad_phiq"
                                                      // (meaningless name)
                                prod += div_corr_2bx2_ref[idim].dot(
                                    grad_phiq[idim]); // [2b_x2] I^2 *
                                                      // convolution = +1.
                                                      // conj(div)*gphij
                            }
                        } // if (||kVaux||^2 > 1e-8)

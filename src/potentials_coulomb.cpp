...
void Potentials::set_sum_of_Vaux(const Parallelization &parallelization,
    ...
    if (method.calc_method() == "FREE") {
        return;
    } // free-electron mode: no interaction

    // not used in HF-BAND.
    if (!(method.calc_method() == "HF" && method.calc_mode() == "BAND")) {
        set_sum_of_Vaux_each(parallelization, crystal_structure, kpoints,
                             kpoints.kvectors_scf(), sum_of_Vaux_scf_);

        // used only when (BI)TC && SCF
        if ((method.calc_method() == "TC" || method.calc_method() == "BITC") &&
            method.calc_mode() == "SCF") {
            set_sum_of_kVaux_each(parallelization, crystal_structure, kpoints,
                                  kpoints.kvectors_scf(), sum_of_kVaux_scf_);
        }
    }
    if (method.calc_mode() == "BAND") {
        set_sum_of_Vaux_each(parallelization, crystal_structure, kpoints,
                             kpoints.kvectors_band(), sum_of_Vaux_band_);

        if (method.calc_method() == "TC" || method.calc_method() == "BITC") {
            // used only when (BI)TC && BAND
            set_sum_of_kVaux_each(parallelization, crystal_structure, kpoints,
                                  kpoints.kvectors_band(), sum_of_kVaux_band_);
        }
    }
}

// private, called in set_sum_of_Vaux
void Potentials::set_sum_of_Vaux_each(
    ...
    const int num_irreducible_kpoints =
        kvectors.size(); // num_irreducible_kpoints_(scf/band)
    ...
    //    alpha_Vaux_ = (alpha_Vaux_ * alpha_Vaux_)/2.0; // alpha =
    //    0.5*V^(2/3)/(4*PI^2)
    alpha_Vaux_ = (alpha_Vaux_ * alpha_Vaux_); // alpha = V^(2/3)/(4*PI^2)

    // set parallelization index (only used here) cf.
    // Parallelization::set_orbital_parallelization()
    std::vector<bool> if_match(num_irreducible_kpoints);
    int ibegin, iend;
    set_parallelization_in_sum_Vaux(parallelization, kpoints, kvectors,
                                    if_match, ibegin, iend);
    ...
    for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
        ...
        for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
            for (int isymq = 0; isymq < kpoints.kvectors_scf()[iq].size();
                 isymq++) {
                if (kpoints.kweight_scf()[iq] < 1e-8) {
                    continue;
                } // zero k-weight (i.e. band k-weight)
                Eigen::Vector3d kq =
                    kvectors[ik][0] - kpoints.kvectors_scf()[iq][isymq];
                ...
                icount++;
                if ((icount - 1) < ibegin || (icount - 1) > iend) {
                    continue;
                } // not assigned to this MPI process

                if (!if_match_kq) // nshell=0 contribution
                {
                    Eigen::Vector3d kqvect =
                        crystal_structure.reciprocal_vectors().transpose() * kq;
    ...
    MPI_Allreduce(&sum_of_Vaux_local[0], &sum_of_Vaux_[0],
                  num_irreducible_kpoints, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
        if (if_match[ik]) // 4pi*exp[-alpha q^2]/q^2 ~ 4pi/q^2 - 4pi*alpha at
                          // q=0 (Note: if_match==true, both nshell=0
                          // contribution in Vaux and 1/0^2 in coulomb are not
                          // included)
        {
            // This is not the correction for \sum_G 4pi*exp[-alpha q^2]/q^2,
            // rather the correction in \int 4pi/q^2 ~ \int 4pi*exp[-alpha
            // q^2]/q^2 + 4pi*alpha for \int dq near q=0. In
            // calc_hamiltonian_two_body_x, we calculate \int - sum_of_Vaux.
            // Thus, "-4pi*alpha" correction for sum_of_Vaux = "+4pi*alpha"
            // correction for \int.
...
void Potentials::set_sum_of_kVaux_each(
    const Parallelization &parallelization,
    const CrystalStructure &crystal_structure, const Kpoints &kpoints,
    const std::vector<std::vector<Eigen::Vector3d>> &kvectors,
    ...
    for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
        ...
        for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
            for (int isymq = 0; isymq < kpoints.kvectors_scf()[iq].size();
                ...
                if (!if_match_kq) // nshell=0 contribution
                {
                    ...
                    double kq2 = kqvect.squaredNorm();
                    sum_of_kVaux_local +=
                        (std::exp(-alpha_Vaux_ * kq2) / kq2) * kqvect;
                }
                for (int nshell = 1;; nshell++) {
                    ...
                    if (sum_on_shell.squaredNorm() < 1e-20) {
                        break;
                    }
                } // int nshell
            }     // isymq
        ...
        MPI_Allreduce(sum_of_kVaux_local.data(), sum_of_kVaux_[ik].data(), 3,
...
void Potentials::set_parallelization_in_sum_Vaux(
    ...
    for (int ik = 0; ik < num_irreducible_kpoints; ik++) {
        ...
        for (int iq = 0; iq < num_irreducible_kpoints_scf; iq++) {
            for (int isymq = 0; isymq < kpoints.kvectors_scf()[iq].size();
                ...
                if (kq.squaredNorm() < 1e-8) {
                    if_match[ik] = true;
                } // always true for the SCF k-mesh
            }
        }
    }
    ...
    std::fill(ncount_each.begin(), ncount_each.end(),
              icount / num_mpi_processes);
